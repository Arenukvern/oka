...
- **`BrowserLauncher`** — the *how*: replaceable seam, three
  ...
  1. **`okaOwned` (default goal)** — oka spawns the binary with
     ...
     This port-only reuse may record a borrowed **process** lease, but it
     has no profile identity and must not create or borrow a profile-state
     lease.
...
Profile launch and cleanup use different evidence thresholds. A valid local
`SingletonLock` whose owner is stopped may permit launching Chrome again, but
the still-present lock is inconclusive for profile deletion; only an absent
lock is affirmative unused evidence.
...
Tracked in `docs/PHASE_CHECKLIST.mdx` (S0–S2).
...
lands), this ADR, `docs/PHASE_CHECKLIST.mdx` (progress).
