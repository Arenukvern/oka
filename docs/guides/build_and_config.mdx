...
[design FAQ](design_faq); settled decisions in
[`docs/decisions/`](../decisions/index).
...
Read the [Gradle migration guide](gradle_migration) — what transfers
...
See [ADR-0008](../decisions/0008-dependency-plan-dry-run).
...
For the release gate, run the project-declared release target. Its verifier
runs `bundletool validate`, can generate a device spec from a connected
device, builds the matching split APK set, and optionally installs it:

```bash
oka run release-google
# target arguments are project-defined; for example:
oka run release-google --oka-target-arg device-spec=test/fixtures/pixel.json
```

The JSON report contains the AAB SHA-256 and byte size, ABIs discovered in the
bundle, generated APK paths, and independent gate results. A target records an
unavailable verifier as a failed or unverified gate rather than silently
claiming store compatibility.
Metadata is intentionally limited to archive metadata unless a future report
consumer supplies an Android manifest dumper.

**Q: How do multi-store Android targets fit into the build?**
Stores are project-declared targets, not build switches. Declare the target
names in `tool/oka_pipeline.dart`, inspect them with `oka explain --targets`,
and run one with `oka run <declared-name>`. Do not add store-specific flags to
`oka build`; the Android pipeline shares dependency resolution, Flutter
assembly, packaging, signing, AAB/APK artifacts, and reports across targets.
See the [multi-store distribution ADR](../decisions/0023-multi-store-distribution-architecture)
and the [publishing guide](publishing) for target setup.

Verification is still store-specific:

- Play checks the release AAB and device-specific split behavior.
- Huawei checks split and universal APK forms and must use the GMS-free
  variant; a GMS-required step fails during composition validation.
- RuStore checks a release-signed AAB and the registered upload-signature
  constraints. A debug-signed or APK-only artifact is not publishable there.

These checks are gates, not claims that a generic Android build is compatible
with every store. A report must record an unavailable verifier as a failed or
unverified gate rather than silently passing.
...
applies R8 shrinking, and signs with the release keystore: `android/key.properties`
...
**Q: How does R8 shrinking work (and what do I need to install)?**
Release builds run the **real R8 shrinker** (tree-shaking + minification) over
the full program closure (app + Flutter embedding + plugin jars). R8 ships via
Google Maven, not build-tools, so oka provisions it into `~/.oka/tools/r8`:

```bash
oka get r8          # or let the build self-heal (OKA_NO_AUTO_INSTALL=1 opts out)
```

R8 is invoked with documented CLI flags only (`--release --min-api --lib
--pg-conf --pg-map-output --pg-conf-output`), verified against R8 9.4.24.
Outputs land under `<buildDir>/r8/`: `mapping.txt` (upload it with your Play
release for deobfuscation) and `configuration.txt` (the merged rules). A
release build fails hard if R8 is missing or produces no mapping — no silent
D8 fallback.

oka's default keep rules cover the reflection surface real apps rely on
(Flutter embedding, plugin registrants, manifest components, JNI `native`
methods, `Parcelable.CREATOR`, enum `values()/valueOf()`, reflection
attributes). Project rules are typed config (ADR-0010):

```dart
AndroidBuild(
  // default: release shrinks. Set false only deliberately:
  // enableOptimization: false,   // keeps R8 + mapping, disables shaking
  proguardFiles: ['android/proguard-rules.pro'],  // merged with defaults
)
```

A missing rule file fails the build; keep `proguard_files` in review.
    ...
    pipelines: [
      AndroidPipeline(
        config: AndroidBuild(          // was android: + top-level name:
          ...
          abis: ['arm64-v8a'],
          javaVersion: 17,
          // Passed to kotlinc separately from kotlin source paths.
          kotlinCompilerArgs: ['-opt-in=kotlin.ExperimentalStdlibApi'],
        ),
...
- Kotlin compiler requirements are declarative: use
  `android.kotlin_compiler_args` in YAML or
  `AndroidBuild.kotlinCompilerArgs` in Dart. `kotlinSources` remains a list of
  source paths and is never used to smuggle compiler flags.
...
validated pipeline machinery. Behavior SSOT: [hot_reload_plan.md](hot_reload_plan),
[ADR-0011](../decisions/0011-hot-reload-run-loop).
...
For disk usage, emulator/simulator storage and scoped cleanup, see
[cache storage and cleanup](cache_storage): `oka cache stats`, then
`oka cache prune --apply` (omit `--apply` to preview).
...
```
It resolves from Google Maven / Maven Central into the shared store's `maven/`
directory (with a legacy fallback; see [cache locations](cache_storage)),
follows the same limited-depth POM transitive closure as plugin dependencies
(ADR-0008), and is dexed into the APK. AAR dependencies also package their
`jni/` natives — filtered to the build's ABIs — and `res/` trees, so host
Kotlin/Java compiled against an extra dep both links and runs (e.g.
MediaPipe `tasks-vision` pulls `tasks-core` classes and native libs without
editing oka).
...
runbook: [contribution guide](../contributing/contribution_guide).
