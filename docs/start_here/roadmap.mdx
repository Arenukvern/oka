...
- **Real store uploads.** `oka_play` and `oka_huawei` compile to validated
  dry-run plans today; `oka_rustore` now provides the typed AAB/signature
  readiness target, while its live upload adapter remains future work.
  Flipping the implemented upload tails to real uploads needs maintainer
...
- **Doctor and cache policy.** Extend the secret audit where real gaps show up.
  Cache last-use tracking would require new metadata and a policy decision;
  existing timestamps are creation/modification times. Storage inspection and
  explicit pruning are covered by [the cache guide](../guides/cache_storage).
...
- **Runtime sessions beyond browsers.** The ADR-0017 handle convention
  extends to `apple/container`/Docker sessions and Dart server-process
  sessions as independent targets. Oka-owned processes keep the
  ADR-0018 lease and teardown safety laws; durable leases are used where a
  resource must outlive its creator and be adopted, not required for every
  process.
- **Declarative resource composition SDK (ADR-0026).** A cross-project API
  may let developers compose processes, harnesses, and protocol sessions
  from typed, replaceable components, with lifecycle reconciliation and
  diagnostic evidence as first-class concerns. Its package boundary and
  language-specific SDKs are gated on pilots with `ecsai_harness`, Oka, and
  IntentCall. Any supervisor remains optional and may be a separate
  implementation or project; this does not change Oka's no-daemon posture.
