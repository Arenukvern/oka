...
ProcessLease lease({
  ...
  identity: {'avd': 'oka-emulator', processLeasePidTokenKey: ?token},
...
void main() {
  ...
  Future<LeaseStopOutcome> stop(final String id, {final bool force = false}) =>
      stopLease(
        temp.path,
        id,
        liveness: liveness,
  ...
  group('inventoryLeases', () {
    test(
      'throwing stop adapter returns failure and retains the lease',
      () async {
        final registry = reg();
        final recorded = lease();
        await registry.upsert(recorded);
        final outcome =
        ...
        expect(outcome.disposition, ProcessStopDisposition.failed);
        expect(outcome.message, contains('adapter unavailable'));
        expect(await registry.read(recorded.id), isNotNull);
        expect(liveness.killed, isEmpty);
      },
    );

    test(
      'lists leases with their reconcile verdicts; empty project → []',
      () async {
        expect(await inventoryLeases(temp.path, liveness: liveness), isEmpty);
        final registry = reg();
        await registry.upsert(lease());
        await registry.upsert(
          lease(id: 'chrome-main', pid: 0, token: null).copyWith(
        ...
        // PID zero is unverifiable; retain the record.
        expect(byId['chrome-main']!.liveness, LeaseLiveness.unknown);
  ...
  group('stopLease laws', () {
    ...
    test(
      'live+owned → graceful hint runs, death verified, record removed',
      () async {
        final registry = reg();
        await registry.upsert(lease());
    ...
    test('pid recycled during grace is never force-signaled', () async {
      final registry = reg();
      await registry.upsert(lease());
      final outcome = await stopLease(
        ...
        'emulator-oka-emulator',
        liveness: liveness,
        stopVerifyGrace: Duration.zero,
        runProcess: (_, _) async {
          liveness.token = 'tok-RECYCLED';
          return ProcessResult(0, 0, '', '');
        },
      );
      expect(outcome.action, LeaseStopAction.recordDropped);
      expect(liveness.killed, isEmpty);
      expect(await registry.list(), isEmpty);
    });

    test(
      'hint fails → force rung rescues (ladder); both fail → failed',
      () async {
        final registry = reg();
        await registry.upsert(lease());
    ...
    test('pid 0 borrowed requires force and verified semantic stop', () async {
      final registry = reg();
      await registry.upsert(
        lease(pid: 0, token: null).copyWith(
      ...
      final o = await stopLease(
        ...
        verifySemanticStop: (_) async => true,
      );
      expect(o.ok, isTrue);
      expect(o.action, LeaseStopAction.stopped);
      expect(hintRuns, ['adb -s emulator-5554 emu kill']);
    ...
    test('pid 0 unverified semantic stop retains the lease', () async {
      final registry = reg();
      await registry.upsert(
        lease(pid: 0, token: null).copyWith(
          identity: const {'serial': 'emulator-5554'},
          ownership: LeaseOwnership.borrowed,
        ),
      );
      final outcome = await stopLease(
      ...
      expect(outcome.action, LeaseStopAction.failed);
      expect(await registry.list(), hasLength(1));
    });
  });
  group('reconcileLeases (L2 sweep)', () {
    ...
    test(
      'provably-stale records are dropped; nothing is ever signaled',
      () async {
        final registry = reg();
        await registry.upsert(lease());
    ...
    test(
      'live owned+ephemeral → orphan-suspect: reported, NOT stopped',
      () async {
        final registry = reg();
        await registry.upsert(lease());
        final summary = await reconcileLeases(temp.path, liveness: liveness);
        ...
        expect(
          ...
          isEmpty,
          reason:
              'inspection-time sweeps never auto-stop: the emulator '
    ...
    test(
      'borrowed and persistent live leases are kept, never touched',
      () async {
        final registry = reg();
        await registry.upsert(
          lease(id: 'a-borrowed').copyWith(
            identity: lease().identity,
            ownership: LeaseOwnership.borrowed,
          ),
        );
