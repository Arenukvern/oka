...
void main() {
  ...
  test(
    'serial never registers → spawned emulator stopped, lease removed',
    () async {
  ...
  test(
    'boot never completes → spawned emulator stopped, lease removed',
    () async {
      if (!_adbAndEmulatorOnPath()) return;
      final fake = FakeRunner()
        ..reply = (_, args) {
  ...
  test('pid recycled mid-run → NOT signaled, lease dropped as stale', () async {
    if (!_adbAndEmulatorOnPath()) return;
    final fake = FakeRunner()
      ..reply = (_, args) {
        final a = args.join(' ');
  ...
  test(
    'adopt path: existing owned lease flips to borrowed, kills nothing',
    () async {
      if (!_adbAndEmulatorOnPath()) return;
      final reg2 = registry;
      await reg2.upsert(
  ...
  test(
    'adopt path with no prior lease records one as borrowed (pid 0)',
    () async {
      if (!_adbAndEmulatorOnPath()) return;
      final reg3 = registry;
      final fake = FakeRunner()
